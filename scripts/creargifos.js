document.addEventListener("DOMContentLoaded", async () => {
  ...
  const gifo_creado = document.getElementsByClassName('crear_gifos_3');
  ...
  const mis_gifos_container = document.getElementById('mis_gifos_container')
  const mis_gifos_section = document.getElementById("mis_gifos_section")
  const $desc = document.querySelector(".a_download");
  let recorder = null
  let blob = null
  ...
  button_capture.addEventListener('click', async () => {
    ...
    input_ready.style.marginTop = "450px"
  ...
  const stopRecord = async (recorder, container) => {
    ...
    input_ready.style.display = "block"
    input_ready.style.marginTop = "5px"
  ...
  const renderMyGifs = gifs => {
    let container = document.querySelector("#mis_gifos_container");
